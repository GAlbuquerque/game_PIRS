...
# Code-only feature flag for the calibration simulation preview in Advanced
# Settings. Set this to True when the internal testing controls are needed.
SHOW_SETTINGS_SIMULATION_TEST = False
...
def _render_settings_page() -> None:
    ...
    with st.container():
        ...
        if SHOW_SETTINGS_SIMULATION_TEST:
            with st.container(border=True):
        ...
        action_cols = st.columns(4 if SHOW_SETTINGS_SIMULATION_TEST else 3)
        play_col = action_cols[0]
        reset_col = action_cols[-2]
        cancel_col = action_cols[-1]
        play = play_col.button("Play", type="primary", width="stretch")
        simulate = (
            action_cols[1].button("Simulate", width="stretch")
            if SHOW_SETTINGS_SIMULATION_TEST
            else False
        )
