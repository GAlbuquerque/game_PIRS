...
central-banker mode.  Before AD--AS, natural unemployment mean-reverts and
$r^*$ follows a random walk:
\begin{align}
 u_t^n&=\max\{2,\,u_{t-1}^n-0.02(u_{t-1}^n-5)+\varepsilon_t^{u^n}\},\\
 r_t^*&=r_{t-1}^*+\varepsilon_t^{r^*}.
\end{align}
...
\begin{equation}
 u_t=\operatorname{clip}_{[1,99]}
       \left(u_{t-1}-0.4(g_t-g^p)\right). \tag{Okun}
\end{equation}
