...
\begin{align}
  ...
  +w_{10}\left[\frac{1}{10}\sum_{j=1}^{10}
    \bigl((i_{t-j}-\pi_{t-j})-\rstar_{t-j}\bigr)\right]
  \notag\\
  &+w_{20}\left[\frac{1}{20}\sum_{j=1}^{20}
    \bigl((i_{t-j}-\pi_{t-j})-\rstar_{t-j}\bigr)\right]
  \notag\\
  &+\beta_r(i_t-\pi_t-\rstar_t)+\varepsilon_{d,t},
...
Both the current and historical real-rate gaps use actual inflation.  A real
rate above $\rstar$ reduces nominal-demand growth; a rate below $\rstar$ raises
it.  The moving averages represent monetary-policy lags.
...
\[
  \frac{\partial\gap_t}{\partial\pi_t}
  =-\frac{1+\beta_r}{4}<0,
  \qquad \beta_r>-1.
\]
